...
class StopWatch extends Component {
  ...
  getMsec(){
    ...
    if(msecs < 100) {
      return `0${parseInt(msecs/10)}`
    } else {
      return `${parseInt(msecs/10)}`
    }
  ...
  render() {
    ...
    return(
      <View ref="stopwatch" style={styles.container}>
        ...
        <Text style={{...styles.text,left: 50}}>{this.getFront()}</Text>
        <Text style={{...styles.text, right: 70}}>{this.getMsec()}</Text>
      </View>
    );
  }
}
